...
export function SignupPage() {
  ...
  const handleBypassAuth = (role: 'admin' | 'client') => {
    ...
    const emailToUse = isValAdmin ? 'sahilrawat399@gmail.com' : (email || 'client@example.com');
    const nameToUse = isValAdmin ? 'Sahil Rawat (Admin)' : (fullName || 'Luxe Client Member');
    ...
    const newProfile = {
      ...
      email: emailToUse,
      role: role,
      businessName: businessName || (isValAdmin ? 'Leopard Luxe Super Admin' : 'Luxe Premium Client'),
    ...
    trackCustomEvent('Developer Bypass Sign Up', { role });
    navigate('/dashboard');
  };
